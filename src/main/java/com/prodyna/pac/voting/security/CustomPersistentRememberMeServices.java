 ...
 * <p>
 * This is a specific implementation of Spring Security's remember-me authentication, but it is much more powerful than the standard
 * implementations:
 * <ul>
...
public class CustomPersistentRememberMeServices extends AbstractRememberMeServices
{
    ...
    @Inject
    public CustomPersistentRememberMeServices(final ApplicationProperties applicationProperties,
    ...
    {
        final PersistentToken token = this.getPersistentToken(cookieTokens);
        final String userName = token.getUser().getUserName();

        // Token also matches, so login is valid. Update the token value, keeping the *same* series number.
        this.log.debug("Refreshing persistent login token for user '{}', series '{}'", userName, token.getSeries());
        ...
        try
        {
            this.persistentTokenRepository.saveAndFlush(token);
            ...
            final Authentication successfulAuthentication)
    {
        ...
        this.log.debug("Creating new persistent login for user {}", userName);
     ...
     * <p>
     * The standard Spring Security implementations are too basic: they invalidate all tokens for the current user, so when he logs out from
     * one browser, all his other sessions are destroyed.
     */
    @Override
    @Transactional
    public void logout(final HttpServletRequest request, final HttpServletResponse response, final Authentication authentication)
    {
        final String rememberMeCookie = this.extractRememberMeCookie(request);
        if ((rememberMeCookie != null) && (rememberMeCookie.length() != 0))
        {
            try
            {
                final String[] cookieTokens = this.decodeCookie(rememberMeCookie);
                final PersistentToken token = this.getPersistentToken(cookieTokens);
    ...
    {
        this.setCookie(new String[] { token.getSeries(), token.getTokenValue() }, TOKEN_VALIDITY_SECONDS, request, response);
    }
}
